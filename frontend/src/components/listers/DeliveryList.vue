<template>
    <div>
        <v-list two-line>
            <template>
                <v-list-item v-for="(data, n) in values" :key="n">
                    <v-list-item-avatar color="grey darken-1">
                        <v-img :src="data.photo ? data.photo:'https://cdn.vuetifyjs.com/images/cards/cooking.png'"/>
                    </v-list-item-avatar>

                    <v-list-item-content>
                        <v-list-item-title style="margin-bottom:10px;">
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                        </v-list-item-title>

                        <v-list-item-subtitle style="font-size:25px; font-weight:700;">
                            [ Id :  {{data.id }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ StoreId :  {{data.storeId }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ MenuId :  {{data.menuId }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ MenuPrice :  {{data.menuPrice }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ CustomerId :  {{data.customerId }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ PayDate :  {{data.payDate }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ CustomerName :  {{data.customerName }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ CustomerAddress :  {{data.customerAddress }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ CustomerPhoneNumber :  {{data.customerPhoneNumber }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ AcceptYn :  {{data.acceptYn }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ CookStartDt :  {{data.cookStartDt }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ PayVanNo :  {{data.payVanNo }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ RiderInfo :  {{data.riderInfo }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ PickDt :  {{data.pickDt }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ FinishCookDt :  {{data.finishCookDt }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            [ FinishDeliveryDt :  {{data.finishDeliveryDt }} ] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        </v-list-item-subtitle>

                    </v-list-item-content>
                </v-list-item>

                <v-divider v-if="n !== 6" :key="`divider-${n}`" inset></v-divider>
            </template>
        </v-list>

        <v-col style="margin-bottom:40px;">
            <div class="text-center">
                <v-dialog
                        v-model="openDialog"
                        width="332.5"
                        fullscreen
                        hide-overlay
                        transition="dialog-bottom-transition"
                >
                    <template v-slot:activator="{ on, attrs }">
                        <v-fab-transition>
                            <v-btn
                                    color="blue"
                                    fab
                                    dark
                                    large
                                    style="position:absolute; bottom: 5%; right: 2%; z-index:99"
                                    @click="openDialog=true;"
                            >
                                <v-icon v-bind="attrs" v-on="on">mdi-plus</v-icon>
                            </v-btn>
                        </v-fab-transition>
                    </template>

                    <Delivery :offline="offline" class="video-card" :isNew="true" :editMode="true" v-model="newValue" @add="append" v-if="tick"/>
                
                    <v-btn
                            style="postition:absolute; top:2%; right:2%"
                            @click="closeDialog()"
                            depressed 
                            icon 
                            absolute
                    >
                        <v-icon>mdi-close</v-icon>
                    </v-btn>
                </v-dialog>
            </div>
        </v-col>
    </div>
</template>

<script>
    const axios = require('axios').default;
    import Delivery from './../Delivery.vue';

    export default {
        name: 'DeliveryManager',
        components: {
            Delivery,
        },
        props: {
            offline: Boolean,
            editMode: Boolean,
            isNew: Boolean
        },
        data: () => ({
            values: [],
            newValue: {},
            tick : true,
            openDialog : false,
        }),
        async created() {
            if(this.offline){
                if(!this.values) this.values = [];
                return;
            } 

            var temp = await axios.get(axios.fixUrl('/deliveries'))
            temp.data._embedded.deliveries.map(obj => obj.id=obj._links.self.href.split("/")[obj._links.self.href.split("/").length - 1])
            this.values = temp.data._embedded.deliveries;
            
            this.newValue = {
                'storeId': '',
                'menuId': '',
                'menuPrice': '',
                'customerId': '',
                'payDate': '2023-03-19',
                'customerName': '',
                'customerAddress': '',
                'customerPhoneNumber': '',
                'acceptYn': false,
                'cookStartDt': '2023-03-19',
                'payVanNo': '2023-03-19',
                'riderInfo': '',
                'pickDt': '2023-03-19',
                'finishCookDt': '2023-03-19',
                'finishDeliveryDt': '2023-03-19',
            }
        },
        methods: {
            closeDialog(){
                this.openDialog = false
            },
            append(value){
                this.tick = false
                this.newValue = {}
                this.values.push(value)
                
                this.$emit('input', this.values);

                this.$nextTick(function(){
                    this.tick=true
                })
            }
        },
    };
</script>


<style>
    .video-card {
        width:300px; 
        margin-left:4.5%; 
        margin-top:50px; 
        margin-bottom:50px;
    }
</style>

